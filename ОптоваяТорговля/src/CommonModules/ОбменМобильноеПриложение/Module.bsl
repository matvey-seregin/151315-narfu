	...
	Попытка
		...
		Для Каждого ДанныеОтвета Из МассивДанных Цикл
			...
			Если СсылкаНаДокумент.ПолучитьОбъект() = Неопределено Тогда
				...
				ДокументЗаказа.УстановитьСсылкуНового(СсылкаНаДокумент);
			Иначе
				ДокументЗаказа = СсылкаНаДокумент.ПолучитьОбъект();
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(ДанныеОтвета.ДатаНачалаРаботы) Тогда
				ДокументЗаказа.ДатаНачалаРаботы = Дата(ДанныеОтвета.ДатаНачалаРаботы);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ДанныеОтвета.ДатаОкончанияРаботы) Тогда
				ДокументЗаказа.ДатаОкончанияРаботы = Дата(ДанныеОтвета.ДатаОкончанияРаботы);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(ДанныеОтвета.ГеоПозиция) Тогда
				ДокументЗаказа.ГеоПозиция = ДанныеОтвета.ГеоПозиция;
			КонецЕсли;
...
Функция ОписаниеКонтрагента(Объект)
	...
	Результат.Вставить("Идентификатор", Строка(Объект.Ссылка.УникальныйИдентификатор()));
	Результат.Вставить("Наименование", Объект.Наименование);
	Результат.Вставить("НомерТелеофна",Объект.НомерТелефона);
